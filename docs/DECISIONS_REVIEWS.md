...
## ADR-REVIEW-005: File-Scoped ADR Numbering Convention

**Decision:** Each decision document uses its own `ADR-<SCOPE>-NNN` prefix where `SCOPE` is the file's topic (e.g., `REVIEW`, `CHAT`, `AUTH`). Numbers restart at `001` per file. All decision documents are written in **English**.

Examples:
- `DECISIONS_REVIEWS.md` → `ADR-REVIEW-001`, `ADR-REVIEW-002`, …
- `DECISIONS_CHAT.md` → `ADR-CHAT-001`, `ADR-CHAT-002`, …
- `DECISIONS_AUTH.md` → `ADR-AUTH-001`, … *(to be migrated from ADR-0xx)*
...
- English is the lingua franca for technical documentation and ensures the decisions are readable by future contributors regardless of native language.

**Rejected alternatives:**
- Global sequence across all files — requires a shared counter, breaks when files are read in isolation.
- No numbering (only titles) — titles change; numbers are stable cross-references (e.g., "see ADR-REVIEW-002").
