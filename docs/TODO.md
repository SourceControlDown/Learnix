...
| B-46.6 | AI chat tools: `get_my_learning_profile`, `get_instructor_courses` | Done | Profile tool is caller-scoped — takes no user id, to keep prompt injection from reading other students. `ILessonProgressRepository.GetProgressCountsAsync` added for bulk progress. See ADR-BACK-CHAT-011 |
