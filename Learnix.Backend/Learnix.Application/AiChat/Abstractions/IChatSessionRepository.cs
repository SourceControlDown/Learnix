...
public interface IChatSessionRepository
{
    ...
    Task<ChatSession?> GetByScopeAsync(Guid userId, ChatScope scope, CancellationToken cancellationToken = default);

    /// <summary>Upserts atomically: a concurrent first message cannot fork the history.</summary>
    Task<ChatSession> GetOrCreateAsync(Guid userId, ChatScope scope, CancellationToken cancellationToken = default);
    ...
    Task DeleteAsync(Guid userId, ChatScope scope, CancellationToken cancellationToken = default);
