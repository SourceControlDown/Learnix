...
public class AdminUnpublishCourseCommandHandlerTests
{
    ...
    {
        _sut = new AdminUnpublishCourseCommandHandler(_currentUserService, _courseRepository, _unitOfWork, _cache);
    }
    ...
    public async Task Handle_ShouldReturnError_WhenUserIsNotAuthenticated()
    {
        ...
        var command = new AdminUnpublishCourseCommand(Guid.NewGuid());
    ...
    public async Task Handle_ShouldReturnError_WhenUserIsNotAdmin()
    {
        ...
        result.Errors[0].Message.Should().Be(CourseMessages.OnlyAdminsForceUnpublish);
    }
