...
public class AdminRecoverCourseCommandHandlerTests
{
    ...
    {
        _sut = new AdminRecoverCourseCommandHandler(_currentUserService, _courseRepository, _unitOfWork, _cache);
    }
    ...
    public async Task Handle_ShouldReturnError_WhenUserIsNotAuthenticated()
    {
        ...
        var command = new AdminRecoverCourseCommand(Guid.NewGuid());
    ...
    public async Task Handle_ShouldReturnError_WhenUserIsNotAdmin()
    {
        ...
        result.Errors[0].Message.Should().Be(CourseMessages.OnlyAdminsRecoverCourses);
    }
