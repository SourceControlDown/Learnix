...
namespace Learnix.Application.UnitTests.Courses.Commands.UnpublishCourse;
...
public class UnpublishCourseCommandHandlerTests
{
    ...
    private readonly UnpublishCourseCommandHandler _sut;

    public UnpublishCourseCommandHandlerTests()
    {
        _sut = new UnpublishCourseCommandHandler(_currentUserService, _courseRepository, _unitOfWork, _cache);
    }
    ...
    public async Task Handle_ShouldReturnError_WhenUserIsNotAuthenticated()
    {
        ...
        var command = new UnpublishCourseCommand(Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_ShouldUnpublishCourseAndClearCache_WhenSuccessful()
    {
        // Arrange
        ...
        var command = new UnpublishCourseCommand(course.Id);
